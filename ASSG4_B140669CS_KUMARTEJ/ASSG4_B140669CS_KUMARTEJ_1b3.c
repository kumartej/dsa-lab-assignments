...
int hash_2(int key){
	double a=key*0.6180339887;
	double b;
	if(key>=0){
	b=(a-(int)a);return (int)(b*c);}
	b=(a+(int)a);
	return (int)(b*c-1);
}

int hash_function(int key,int probe){
	int a=(hash_1(key)+probe*hash_2(key))%c;
	if(a>=0)
	return a;
	else
	return (c+a)%c;
...
/*data* insert(data * h,int k){
	int hkey1=hash_1(k),hkey2=hash_2(k),j=1,hk;
	printf("%d ",hkey1);
	hk=hkey1;
	while(j<c && h[hkey1].key!=NULL){
	    hkey1=(hkey1+hkey2)%c;
	    printf("%d ",hkey1);j++;
	}
	//printf("\n");
	if(h[hkey1].key==NULL){
	    h[hkey1].key=intkey(k);
	    printf("\n");
	    return h;
	}
	else 
